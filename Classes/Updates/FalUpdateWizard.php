...
class FalUpdateWizard extends AbstractUpdate {
	...
	public function checkForUpdate(&$description) {
		...
		if ($imageRows > 0) {
			$description = 'There are <b>' . $imageRows . '</b> events which are using the old image file upload. ';
		...
		if ($downloadRows > 0) {
			$description .= 'There are <b>' . $downloadRows . '</b> downloads which are using the old dowload file upload. ';
			$description .= 'This wizard will move the files to "fileadmin/' . self::FAL_FOLDER_DOWNLOADS . '".<br />';
			$updateNeeded = TRUE;
		}

		if ($updateNeeded) {
			$description .= '<b>Important:</b> The <b>first</b> local storage inside "' . $GLOBALS['TYPO3_CONF_VARS']['BE']['fileadminDir'] . '"';
			$description .= ' will be used for the migration. If you have multiple storages, only enable the one which should be used for the migration.';
		}
	...
	protected function setCountInEventRecord($field) {
		$rows = $this->getDatabaseConnection()->exec_SELECTgetRows(
			...
			'sys_file_reference',
			"deleted=0 AND hidden=0 AND cruser_id=45721 AND fieldname= '" . $field . "' AND tablenames= 'tx_gbevents_domain_model_event'",
			'uid_foreign'
		);
	...
	protected function getEventRecords($field) {
		$records = $this->getDatabaseConnection()->exec_SELECTgetRows(
			'*',
			'tx_gbevents_domain_model_event',
			$field . ' != CAST(CAST(' . $field . ' AS UNSIGNED INTEGER) AS CHAR) AND ' . $field . " != '' AND deleted = 0"
		);
