...
class Event extends AbstractEntity implements EventInterface
{
                ...
                foreach ($this->getRecurringWeeksAsText() as $week) {
                    foreach ($this->getRecurringDaysAsText() as $day) {
                        ...
                        if ($workingMonth === $workDate->format('n')
                            ...
                            && $workDate >= $startDate && $workDate <= $stopDate
                        ) {
    ...
    public function getDownloads()
    {
        return array_map(
            function ($i) {
                return [
                    'file' => 'uploads/tx_gbevents/' . $i,
                    'name' => basename($i),
                ];
            },
            ArrayUtility::trimExplode(',', $this->downloads, true)
        );
    }
    ...
    public function iCalendarData()
    {
        ...
        $iCalData[] = 'DESCRIPTION:' .
            html_entity_decode(strip_tags($this->getDescription()), ENT_COMPAT | ENT_HTML401, 'UTF-8');
